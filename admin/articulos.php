...
<a href="articulos_form.php" class="btn btn-success mb-3">
    ➕ Nuevo artículo
</a>
<div class="card">
    <div class="card-body">
        <table class="table table-hover table-striped">
            ...
            <?php while($f = $res->fetch_object()): ?>
                <tr>
                    <td><?= $f->idarticulos ?></td>
                    <td><strong><?= htmlspecialchars($f->articulo) ?></strong></td>
                    <td><strong><?= htmlspecialchars($f->familia) ?></strong></td>
                    <td class="text-end">
                        <a href="articulos_form.php?id=<?= $f->idarticulos ?>"
                           class="btn btn-sm btn-warning">
                            ✏️ Editar
                        </a>
                        <a href="articulos_delete.php?id=<?= $f->idarticulos ?>"
                           class="btn btn-sm btn-danger"
                           onclick="return confirm('⚠️ ¿Eliminar articulo?\n\n')">
                            🗑️ Borrar
                        </a>
                    </td>
                </tr>
